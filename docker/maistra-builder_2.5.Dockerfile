...
RUN curl -sfL https://download.docker.com/linux/centos/docker-ce.repo -o /etc/yum.repos.d/docker-ce.repo && \
    ...
    dnf -y install --setopt=install_weak_deps=False \
                   ...
                   autoconf automake libtool python2 libstdc++-static \
                   java-11-openjdk-devel jq file diffutils lbzip2 \
                   ruby-devel zlib-devel python2-setuptools gcc-toolset-12-libatomic-devel \
                   clang-devel-0:14.0.6-1.module_el8.7.0+1198+0c3eb6e2 llvm-devel-0:14.0.6-1.module_el8.7.0+1198+0c3eb6e2 \
...
RUN git clone https://github.com/openssl/openssl.git && \
    ...
    cp /usr/local/lib64/libcrypto.so.3 /usr/lib64 && \
    cp /usr/local/lib64/libssl.so.3 /usr/lib64 && \
...
RUN mkdir -p /tmp/cmake && \
    cd /tmp/cmake && \
    curl -o cmake-3.25.2.tar.gz -Ls http://www.cmake.org/files/v3.25/cmake-3.25.2.tar.gz && \
    ...
    make install && \
    rm -f /usr/bin/cmake && \
    ln -s /opt/cmake/bin/cmake /usr/bin && \
